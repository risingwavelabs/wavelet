...
Users define streams and views in `wavelet.config.ts`. These need to be synced to RisingWave as tables, materialized views, and subscriptions. Without a DDL manager, users would need to manually run SQL against RisingWave, which defeats the purpose of Wavelet.
...
2. For each stream in config: CREATE TABLE if missing
3. For each view in config:
...
4. For views removed from config: DROP SUBSCRIPTION + DROP MV
5. For streams removed from config: DROP TABLE (only if no MV depends on it)
...
Tables are only dropped if no materialized view references them. This prevents accidentally breaking views that depend on the table.
