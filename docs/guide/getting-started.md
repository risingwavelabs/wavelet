...
**Events** define where data comes in. Each event becomes a table in RisingWave.

**Queries** define what to compute. Each query becomes a materialized view that RisingWave keeps up to date incrementally.
...
2. Syncs your events and queries to RisingWave
...
```typescript
await fetch('http://localhost:8080/v1/events/game_events', {
  method: 'POST',
...
This creates `.wavelet/client.ts` with full TypeScript types derived from your query schemas. Your IDE and AI coding agents get autocomplete for every field.
